...
async def like_target(
    ...
    try:
        result = await use_cases.like(user.id, target_type=target_type, target_id=target_id)
        await discovery_use_cases.record_like_events(
            user.id,
...
async def unlike_target(
    ...
    try:
        result = await use_cases.unlike(user.id, target_type=target_type, target_id=target_id)
        await db_session.commit()
    except ValidationError as exc:
        await db_session.rollback()
        raise _http_error(status.HTTP_400_BAD_REQUEST, "validation_error", exc.message) from exc
...
async def comment_target(
    ...
    try:
        ...
        comment, counters = await use_cases.comment(
            user.id,
            target_type=target_type,
            target_id=target_id,
            body=payload.body,
        ...
        await discovery_use_cases.record_comment_events(
            user.id,
            target_type=target_type,
            target_id=target_id,
        )
...
async def list_comments(
    ...
    try:
        items = await use_cases.list_comments(
            target_type=target_type,
            target_id=target_id,
            limit=limit,
            offset=offset,
...
async def get_comment(
    ...
    try:
        item = await use_cases.get_comment(
            target_type=target_type,
            target_id=target_id,
