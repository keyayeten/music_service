...
class CatalogTrackUseCases:
    ...
    async def create_track(
        self, actor_user_id: UUID, *, title: str, description: str | None, duration_seconds: int
    ) -> TrackReadModel:
    ...
    async def set_track_authors(
        self, actor_user_id: UUID, *, track_id: UUID, author_profile_ids: list[UUID]
    ) -> TrackReadModel:
        composer_profile_id = await self._require_composer_profile(actor_user_id)
        track = await self._require_owned_track(track_id, composer_profile_id)
        if track.status not in {"draft", "rejected", "hidden"}:
            raise ValidationError(
                "Authors can be changed only for draft, rejected or hidden tracks."
            )
    ...
    async def submit_track_for_review(
        self, actor_user_id: UUID, *, track_id: UUID
    ) -> TrackReadModel:
        composer_profile_id = await self._require_composer_profile(actor_user_id)
        track = await self._require_owned_track(track_id, composer_profile_id)
    ...
    async def moderate_track(
        self, actor_roles: list[str], *, track_id: UUID, target_status: str
    ) -> TrackReadModel:
    ...
    async def _require_owned_track(
        self, track_id: UUID, composer_profile_id: UUID
    ) -> TrackReadModel:
        track = await self._repository.get_track_by_id(track_id)
        if track is None:
            raise ValidationError("Track is not found.")
