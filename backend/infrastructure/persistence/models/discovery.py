...
class UserTrackEvent(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            f"event_type IN {TRACK_EVENT_TYPES}", name="ck_user_track_events_event_type"
        ),
...
class ExternalLinkClick(Base):
    ...
    external_link_id: Mapped[UUID] = mapped_column(
        ForeignKey("external_links.id", ondelete="CASCADE"), nullable=False
    )
    track_id: Mapped[UUID] = mapped_column(
        ForeignKey("tracks.id", ondelete="CASCADE"), nullable=False
    )
