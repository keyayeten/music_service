...
class _FakeDiscoveryRepository:
    ...
    def get_track_ids_for_target(self, *, target_type: str, target_id: UUID) -> list[UUID]:
        return self.get_track_ids_for_item(item_type=target_type, item_id=target_id)
...
def test_record_save_events_for_album_produces_event_per_track() -> None:
    ...
    use_cases.record_save_events(actor_user_id, item_type="album", item_id=uuid4())
...
def test_record_view_events_rejects_empty_track_collection() -> None:
    ...
    with pytest.raises(ValidationError):
        use_cases.record_view_events(uuid4(), track_ids=[])
...
def test_recommendations_use_personalized_or_top_fallback() -> None:
    ...
    personalized = use_cases.list_recommended_tracks(actor_user_id=uuid4(), limit=20)
    fallback = use_cases.list_recommended_tracks(actor_user_id=None, limit=20)
