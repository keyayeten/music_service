...
class _FakeSocialRepository:
    ...
    def update_target_counters(
        ...
        *,
        ...
        current = self.get_target(target_type, target_id)
        if current is None:
            return None
        self.target = SocialTargetReadModel(
...
def test_like_is_idempotent_and_counter_updates_once() -> None:
    ...
    first = use_cases.like(repository.user_id, target_type="track", target_id=repository.target_id)
    second = use_cases.like(repository.user_id, target_type="track", target_id=repository.target_id)
...
def test_unlike_never_makes_counter_negative() -> None:
    ...
    result = use_cases.unlike(repository.user_id, target_type="track", target_id=repository.target_id)
...
def test_get_and_list_comments_return_only_target_comments() -> None:
    ...
    got = use_cases.get_comment(target_type="track", target_id=repository.target_id, comment_id=comment.id)
    listed = use_cases.list_comments(target_type="track", target_id=repository.target_id, limit=20, offset=0)
